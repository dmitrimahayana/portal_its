<div id="agenda-lengkap" class="left corner-top content-based">
	...
	function pagination(pages) {
		...
		$.ajax({
			type: "POST",
			url: "<?php echo base_url(); ?>ajax/ajax_pengumuman/" + tahun + "/" + bulan + "/<?php echo $lang; ?>" + "/" + offset + "/" + num,
			cache:false,
			dataType: "json",
				...
				$.each(msg, function(index, news) {
					html_builder += (
						...
						+'<div class="news-detail ui-accordion-content ui-widget-content ui-accordion-content-active">' 
							+ '<a href="' + news.url + '"><h3 class="news-headline">' + <?php if($lang == 'id') {echo 'news.indonesia';} else {echo 'news.inggris'; } ?> + '</h3> </a>'
							+ '<div class="news-body">Tanggal: '+ news.date +'</div>'
	...
	function count_ajax(tahun, bulan, page_now)
	{
		...
		$.ajax({
			type: "POST",
			url: "<?php echo base_url(); ?>ajax/ajax_count_pengumuman/"+ tahun + "/" + bulan,
			cache:false,
			dataType: "json",
			success: function(msg) {
				...
				if( page_now-3 > 1)
				{
					pagination_links += "<a href=\"#\" onclick=\"javascript:pagination(1);\" "; 		// open tag
				...
				for(var i=1; i<=batas_iterasi_halaman; i++)
				{
					...
					{
						pagination_links += "<a href=\"#\" onclick=\"javascript:pagination("+(i)+");\" "; // open tag
				...
				{
					pagination_links += "<a href=\"#\" onclick=\"javascript:pagination("+(batas_iterasi_halaman)+");\" "; 	// open tag
					pagination_links += "\">"; 														// close open tag
	...
	function generate_month(tahun)
	{
		...
		$.ajax({
			type: "POST",
			url: "<?php echo base_url(); ?>ajax/ajax_month_pengumuman/" + tahun,
			cache:false,
			dataType: "json",
			success: function(msg) {
