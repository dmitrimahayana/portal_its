...
class View_jurusan_model extends Base_lang_model {
        ...
        public function updtIDEN($id){
            ...
            $headOfDept=$this->input->post('headOfDept');
            $mediaImages=$this->input->post('media_id');
            $this->update($id,$no_urut,$namaJurID,$fakultasID,$alamatID,$telp,$fax,$email,$website,$headOfDept,$aboutID,$mediaImages);
            $this->update($id+1,$no_urut,$namaJurEN,$fakultasEN,$alamatEN,$telp,$fax,$email,$website,$headOfDept,$aboutEN,$mediaImages);
        }
        ...
        function get_all()
	{
		...
		$used->distinct();
		$used->select('idJur, no_urut, namaJur as namaJurusan, fakultas, alamat, telp, fax, email, website, headOfDept, about, mediaImages, m.link_thumb as link_thumb');
		$used->from('jurusan');
        ...
        function get_some($fakultas){
            $used = $this->load->database('its', TRUE);
            $used->distinct();
            $used->select('idJur, no_urut, namaJur as namaJurusan, fakultas, alamat, telp, fax, email, website, headOfDept, about, mediaImages');
            $used->from('jurusan');
        ...
        function get_id($id)
        {
            $used = $this->load->database('its', TRUE);
            $used->distinct();
            $used->select('idJur, no_urut,lang, namaJur as namaJurusan, fakultas, alamat, telp, fax, email, website, headOfDept, about, mediaImages');
            $used->from('jurusan');
            $used->where('idJur', $id);
        ...
        function update($id,$no_urut,$namaJur,$namaFak,$alamat,$telp,$fax,$email,$website,$headOfDept,$about,$mediaImages){
            $used = $this->load->database('its', TRUE);
            $sql = "UPDATE jurusan SET no_urut='$no_urut', namaJur='$namaJur', fakultas='$namaFak', alamat='$alamat', telp='$telp', fax='$fax', email='$email', website='$website', headOfDept='$headOfDept', about='$about', mediaImages='$mediaImages' where idJur='$id'";
            $used->query($sql);
        }
